...
describe('FeatureFlagGuard', () => {
  ...
  beforeEach(() => {
    ...
    guard = new FeatureFlagGuard(reflector as Reflector, flags as FeatureFlagsService);
  });

  it('allows when no metadata present', async () => {
    (reflector.get as jest.Mock).mockReturnValue(undefined);
    const mockCtx = { getHandler: () => {}, getClass: () => {}, switchToHttp: () => ({ getRequest: () => ({}) }) } as unknown as ExecutionContext;
  ...
  it('allows when flag enabled', async () => {
    ...
    const mockCtx = { getHandler: () => {}, getClass: () => {}, switchToHttp: () => ({ getRequest: () => ({}) }) } as unknown as ExecutionContext;
    await expect(guard.canActivate(mockCtx)).resolves.toBe(true);
    expect(flags.isEnabled).toHaveBeenCalledWith('some.flag', expect.any(Object));
  });
